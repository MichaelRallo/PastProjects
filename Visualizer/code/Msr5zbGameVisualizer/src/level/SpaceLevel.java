...
public class SpaceLevel implements LevelStandards {
    ...
    private ArrayList<AsteroidMissle> missles;
    
    private int enemyCounter= 0;
    private ArrayList<AsteroidEnemy> enemies;
    ...
    public void pause()
    {
           for (AsteroidEnemy enemy : enemies) 
           {                  
               enemy.paused = true;
           }
           for (AsteroidMissle missle : missles) 
           {                  
               missle.paused = true;
           } 
    }
    ...
    public void start(Integer numBands, AnchorPane vizPane) 
    {
        ...
        //Background Image
        String image = Msr5zbGameVisualizer.class.getResource("images/spaceBackground.gif").toExternalForm();
        try
        {
    ...
    public void update(double timestamp, double duration, float[] magnitudes, float[] phases) 
    {
        ...
        //scoreOutput.setText(Integer.toString(min) + ":" + Integer.toString(sec));
        ...
        //Enemy Spawner
        if(sec%5 == 0 && enemyCounter < 7)
        {
            Random rand = new Random();
            int randomNum = rand.nextInt((500 - 50) + 1) + 50;
            
            AsteroidEnemy enemy = new AsteroidEnemy(vizPane, (double)randomNum); 
            enemies.add(enemy);
            enemy.start();
            enemyCounter++;
        }
        ...
        //Shoot AsteroidMissle
        if(isPressed(KeyCode.W) && missleCount < 1)
        {   System.out.println("W is being Pressed!");
            AsteroidMissle missle = new AsteroidMissle(vizPane, playerRectangle.getTranslateX(), playerRectangle.getTranslateY()); 
            missles.add(missle);
            missle.start();
            missleCount++;              
        }
        
        //If Player Outside Map For Some Reason... Put Him Back
        if(playerRectangle.getTranslateX() <= 0 || playerRectangle.getTranslateX() >= 530)
        {
            playerRectangle.setTranslateX(265);
        }
        
        //Check To See If Time To Clean AsteroidMissles
        for(AsteroidMissle missle : missles) 
        {   
            if(missle != null && missle.stop ==false)
            {
                ...
                //Check To See If AsteroidMissle Hit An Asteroid!
                for(AsteroidEnemy enemy : enemies) 
                {  
       ...
       //Check To See If Asteroid Made It Behind Line
       for(AsteroidEnemy enemy : enemies) 
       {  
    ...
    private void movePlayerX(int value)
    {
        ...
        for(int i = 0; i < Math.abs(value); i++)
        {
            ...
            {
                 playerRectangle.setTranslateX(playerRectangle.getTranslateX() + (movingRight ? -5: 5));
            }
            
            else
            {
                playerRectangle.setTranslateX(playerRectangle.getTranslateX() + (movingRight ? 1: -1));
            }            
        }            
    }
