...
public class TechnoLevel implements LevelStandards {
    ...
    private Double startHue = 1000.0;
    ...
    private ArrayList<TechnoMissle> missles;
    
    private int enemyCounter= 0;
    private ArrayList<TechnoEnemy> enemies;
    ...
    public void pause()
    {
           for (TechnoEnemy enemy : enemies) 
           {                  
               enemy.paused = true;
           }
           for (TechnoMissle missle : missles) 
           {                  
               missle.paused = true;
           } 
    }
    ...
    public void start(Integer numBands, AnchorPane vizPane) 
    {
        ...
        //Background Image
        String image = Msr5zbGameVisualizer.class.getResource("images/background.gif").toExternalForm();
        try
        {
    ...
    public void update(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        //Enemy Spawner
        if(sec%5 == 0 && enemyCounter < 5)
        {
            Random rand = new Random();
            int randomNum = rand.nextInt((500 - 50) + 1) + 50;
            
            TechnoEnemy enemy = new TechnoEnemy(vizPane, (double)randomNum); 
            enemies.add(enemy);
            enemy.start();
            enemyCounter++;
        }
        ...
        {   System.out.println("W is being Pressed!");
            TechnoMissle missle = new TechnoMissle(vizPane, playerRectangle.getTranslateX(), playerRectangle.getTranslateY()); 
            missles.add(missle);
            missle.start();
            missleCount++;              
        }
        
        //If Player Outside Map For Some Reason... Put Them Back
        if(playerRectangle.getTranslateX() <= 0 || playerRectangle.getTranslateX() >= 530)
        {
            playerRectangle.setTranslateX(265);
        }
        
        //Check To See If Time To Clean Missles
        for(TechnoMissle missle : missles) 
        {   
            if(missle != null && missle.stop ==false)
            {
                ...
                //Check To See If Missle Hit An Enemy!
                for(TechnoEnemy enemy : enemies) 
                {  
       ...
       //Check To See If Spider Made It Behind Line, If It Did, Subtract Points
       for(TechnoEnemy enemy : enemies) 
       {  
